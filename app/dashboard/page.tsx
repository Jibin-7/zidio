...
export default async function DashboardPage() {
  ...
  const newThisWeek = await prisma.feedback.count({ where: { workspaceId, createdAt: { gte: sevenDaysAgo } } })
  const negCount = await prisma.feedback.count({ where: { workspaceId, sentiment: 'NEG' } })
  const percentNegative = totalFeedback > 0 ? Math.round((negCount / totalFeedback) * 100) : 0
...
const sentimentData = sentimentGroups.map(
  ...
  const volumeData = Array.from(volumeMap.entries()).map(([name, count]) => ({ name, count }))
